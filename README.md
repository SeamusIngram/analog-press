...
<br /> There are still some disparities, resulting in techniques being much easier to perform than on a Gamecube Controller. For example, pivot tilts are easy; you just need to press the direction you wish to tilt, along with the slow modifier to get your tilt input (pivoting is the challenging part of the technique). Dashback out of crouch is straighforward, because you roll the stick from down to the side (Press down. While still holding down press to the side. Let go of down). The speed at which the stick makes it easy to perform. With some consideration, I think these can be balanced as well. If the DTA scheme is designed to work like an analog stick, then in theory it should have the same capabilites as an analog stick. The design just needs to provide a close enough analogue. Balance is obviously top of mind. A new DTA scheme cannot solve the issues of current inputs but then create new issues at the same time.
